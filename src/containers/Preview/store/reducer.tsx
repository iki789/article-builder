...
export const PreviewReducer: Reducer = (
  state: IPreviewState = initialState,
  action
) => {
  if (action.type === ActionTypes.ADD_ITEM_TO_ROW) {
    ...
    if (action.payload === "embed") {
      data = {
        code:
          '<iframe width="1280" height="720" src="https://www.youtube.com/embed/YE7VzlLtp-4" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>',
      };
    }
  ...
  if (action.type === ActionTypes.UPDATE_COL) {
    ...
    rows.map((row: IRow) => {
      row.cols.map((col: ICol) => {
        if (col && action.payload && col.id === action.payload.id) {
          col.data = action.payload.data;
        }
        return col;
      });
      return row;
